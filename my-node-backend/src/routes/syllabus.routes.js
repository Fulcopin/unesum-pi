...
const TEACHER_ROLES = ['profesor', 'docente'];
const TEACHER_EDITOR_ROLES = [...TEACHER_ROLES, 'administrador', 'comision_academica'];
const TEACHER_CREATOR_ROLES = [...TEACHER_ROLES, 'administrador', 'comision_academica', 'comision'];
...
router.post('/upload', authorize(TEACHER_EDITOR_ROLES), upload.single('file'), syllabusController.uploadDocument);

// POST /api/syllabi/upload-excel -> Subir archivo Excel y crear syllabus automáticamente
// PERMITIDO PARA: profesores, comisión académica y administradores.
router.post('/upload-excel', authorize(TEACHER_EDITOR_ROLES), upload.single('file'), syllabusController.uploadExcel);

// POST /api/syllabi/extract-word-tables -> Extraer tablas crudas de un .docx (mammoth+cheerio)
// PERMITIDO PARA: profesores, comisión académica y administradores.
router.post('/extract-word-tables', authorize(TEACHER_EDITOR_ROLES), upload.single('file'), syllabusController.extractWordTables);
...
router.get('/verificar-existencia', authorize(TEACHER_EDITOR_ROLES), syllabusController.verificarExistencia);
...
router.get('/mine', authorize([...TEACHER_ROLES, 'administrador']), syllabusController.getMine);
...
router.post('/', authorize(TEACHER_CREATOR_ROLES), syllabusController.create);
...
router.get('/:id', authorize([...TEACHER_CREATOR_ROLES]), syllabusController.getById);
...
router.put('/:id', authorize(TEACHER_EDITOR_ROLES), syllabusController.update);

// DELETE /api/syllabi/:id -> Eliminar (soft delete) un syllabus.
// PERMITIDO PARA: Todos los roles. La lógica de "quién es el dueño" está dentro del controlador.
router.delete('/:id', authorize(TEACHER_EDITOR_ROLES), syllabusController.delete);
...
router.post('/upload-validado', authorize([...TEACHER_ROLES, 'comision', 'comision_academica']), upload.single('file'), syllabusController.subirSyllabusConValidacion);

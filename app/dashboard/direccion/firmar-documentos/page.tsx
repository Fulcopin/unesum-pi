...
function DireccionBulkFirmaContent() {
  ...
  const [miQR, setMiQR] = useState<{ qr_data_url: string } | null>(null);

  const etapaUsuario = 'director_academico';
  ...
  useEffect(() => { cargarDocs(); }, [filtroPeriodo, filtroTipo]);
  ...
  const toggleSeleccion = (d: DocPendiente) => {
    setSeleccionados(prev => {
      ...
      if (next.has(clave(d))) next.delete(clave(d)); else next.add(clave(d));
      return next;
    });
  };
  ...
  const ejecutarFirmaMasiva = async () => {
    try {
      ...
      for (const [tipo, ids] of [['syllabus', syllabuIds], ['programa_analitico', paIds]] as [string, number[]][]) {
        if (ids.length === 0) continue;
        const res = await fetch(`${API_URL}/firmas/firmar-masivo`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ tipo, ids, observaciones }),
        });
        const json = await res.json();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <Button variant="ghost" size="sm" onClick={() => router.push('/dashboard/direccion')} className="mb-2">
              <ArrowLeft className="h-4 w-4 mr-1" /> Volver
            </Button>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <PenLine className="h-6 w-6 text-indigo-600" />
              Firma de documentos — Director/a Académico/a
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              Selecciona y firma todos los documentos pendientes de tu visado como Director/a Académico/a.
            </p>
          </div>
          {miQR && (
        ...
        {visibles.length > 0 && (
          <div className="rounded-xl bg-indigo-50 border border-indigo-200 p-3 mb-4 flex items-center gap-3 flex-wrap">
            <Checkbox checked={todosSeleccionados} onCheckedChange={c => c ? seleccionarTodos() : deseleccionarTodos()} id="sel-all" />
            <Label htmlFor="sel-all" className="text-sm font-medium cursor-pointer">Seleccionar todos ({visibles.length})</Label>
            {seleccionados.size > 0 && (
              <>
                <Badge className="bg-indigo-600 hover:bg-indigo-700">{seleccionados.size} seleccionado(s)</Badge>
                <div className="ml-auto">
                  <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={() => setShowConfirm(true)} disabled={firmandoMasivo}>
        ...
        {!loading && visibles.length === 0 && !error && (
          <Card>
            <CardContent className="py-12 text-center text-slate-500">
              ...
              <p className="text-sm mt-1 text-slate-400">Todos los documentos están firmados o aún no es tu turno.</p>
            </CardContent>
          </Card>
        )}
        ...
        {!loading && visibles.length > 0 && (
          <div className="space-y-2">
            {visibles.map(d => {
              ...
              return (
                <div key={key} className={`rounded-xl border p-3 bg-white flex items-center gap-3 cursor-pointer transition-all ${sel ? 'border-indigo-400 bg-indigo-50 shadow-sm' : 'border-slate-200 hover:border-slate-300'}`} onClick={() => toggleSeleccion(d)}>
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Vas a firmar <strong>{seleccionados.size}</strong> documento(s) como <strong>Director/a Académico/a</strong>. Esta acción no se puede deshacer.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            {miQR && (
              <div className="flex items-center gap-3 rounded-lg bg-indigo-50 border border-indigo-200 p-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-indigo-800">Tu sello digital será registrado</p>
                  <p className="text-[10px] text-indigo-600 mt-0.5">Visado final de Director/a Académico/a.</p>
                </div>
              </div>
            )}
            <div>
              <Label className="text-xs">Observaciones (opcional)</Label>
              <Textarea value={observaciones} onChange={e => setObservaciones(e.target.value)} placeholder="Visado y aprobado..." rows={3} className="mt-1" />
            </div>
...
export default function DireccionFirmarDocumentosPage() {
  return (
    <ProtectedRoute allowedRoles={['direccion', 'administrador']}>
      <DireccionBulkFirmaContent />
    </ProtectedRoute>
  );
}
